...
cirname = '4 input NAND-gate cascaded with an inverter '
...
file_voltage = './Data/4inpNAND_inv_voltage.txt'
file_current = './Data/4inpNAND_inv_current.txt'
file_netlist = './Data/4inpNAND_inv.net'
input_list = ['ainp']
output_list = ['invout']
denominator = 40
...
def get_nonlinear_matrix(state,regions,Vth):
	...
	vdot=( I[ 'm9' ] - I[ 'm10' ] - ( state[ 'invout' ] / 100e3 ) )* 1e15
...
def get_stateorder(state):
	...
	stateorder=sympy.Matrix([(state['nandout']),(state['n001']),(state['n002']),(state['n003']),(state['invout'])])
